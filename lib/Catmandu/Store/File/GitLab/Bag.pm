...
sub generator {
    ...
    # my $ns_prefix   = $gitlab->{namespace};
    # my $pid         = "$ns_prefix:$key";
    # my $dsnamespace = $gitlab->{dsnamespace};

    $self->log->debug("Listing datastreams for $pid");

    my $response = $gitlab->listDatastreams(pid => $pid);
...
sub delete {
    ...
    if ($gitlab->{purge}) {
        ...
        $response = $gitlab->purgeDatastream(pid => $pid, dsID => $dsid);
    }
    else {
        $self->log->debug("Set datastream state D $pid:$dsid");
        $response = $gitlab->setDatastreamState(
            pid     => $pid,
            dsID    => $dsid,
            dsState => 'D'
        );
...
sub _get {
    ...
    my $response = $gitlab->getDatastreamHistory(pid => $pid, dsID => $dsid);
    ...
    return {
        ...
        _stream      => sub {
            my $out  = shift;
            my $bytes = 0;
            my $res = $gitlab->getDatastreamDissemination(
                pid      => $pid,
                dsID     => $dsid,
                callback => sub {
...
sub _add_filename {
    ...
    my $pid         = "$ns_prefix:" . $self->name;
    my $dsnamespace = $gitlab->{dsnamespace};
    my $versionable = $gitlab->{versionable} ? 'true' : 'false';

    my %options = ('versionable' => $versionable);

    if ($gitlab->{md5enabled}) {
    ...
    if ($operation eq 'ADD') {
        ...
        $response = $gitlab->modifyDatastream(
            pid      => $pid,
            dsID     => $dsid,
            file     => $filename,
            dsLabel  => $key,
...
sub _add_stream {
    ...
    my ($fh, $filename)
        = File::Temp::tempfile(
        "librecat-filestore-container-gitlabcommons-XXXX",
        UNLINK => 1);
